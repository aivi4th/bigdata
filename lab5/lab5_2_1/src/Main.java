...
class Student {
    ...
    public void setBirth(LocalDate birth){
        if (birth.isAfter(LocalDate.now()))
            throw new IllegalArgumentException("Date of birth should be before now");
        this.birth = birth;
    }
    ...
    public void setFacultyCourseGroup(String faculty, int course, String group) {
        if (!Constants.FACULTIES.contains(faculty))
            throw new IllegalArgumentException("There are no such faculty: " + faculty);
        this.faculty = faculty;
        if (course < 1 || course > 6)
            throw new IllegalArgumentException("There are no such course: " + course);
        this.course = course;
        if (!group.matches("^\\d.*"))
            throw new IllegalArgumentException("Group name should start with a number");
        this.group = group;
    }
...
class Constants {
    public static final List<String> FACULTIES = Arrays.asList("IU", "IBM", "SM", "E", "MT", "BMT", "");
}